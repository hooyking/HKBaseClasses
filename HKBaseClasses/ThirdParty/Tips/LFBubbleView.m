...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        UIView *bgVi = [[UIView alloc] initWithFrame:kAppWindow.bounds];
        bgVi.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0];
        [kAppWindow addSubview:bgVi];
        ...
        [bgVi addGestureRecognizer:tapGR];
        ...
        UITapGestureRecognizer *tapGR1 = [[UITapGestureRecognizer alloc] initWithActionBlock:^(id  _Nonnull sender) {
            [self removeFromSuperview];
            [bgVi removeFromSuperview];
        }];
...
+ (void)showExplainWithTargetView:(UIView *)targetView
                             text:(NSString *)text
                         hostView:(UIView *)hostView {
    
    CGPoint point  = [[targetView superview] convertPoint:targetView.center toView:hostView];
    CGFloat space = 1.0f/3.0f;
    if (point.x+170>kScreenW) {
        space = 2.5f/3.0f;
    }
    LFBubbleView *bubbleView  = [[LFBubbleView alloc] initWithFrame:CGRectMake(0, 0, 180, [text stringSizeWithSize:CGSizeMake(160, CGFLOAT_MAX) font:[UIFont systemFontOfSize:12]].height+20)];
    bubbleView.direction = LFTriangleDirection_Up;
    bubbleView.lbTitle.text = text;
    bubbleView.triangleXYScale = space;
    [hostView addSubview:bubbleView];
    [bubbleView showInPoint:CGPointMake(point.x, point.y+6)];
}
