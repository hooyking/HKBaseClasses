...
- (UITextField *)textField
{
    if (!_textField)
    {
        _textField = [[UITextField alloc] initWithFrame:CGRectMake(12, 7, self.frame.size.width-12*2, 30)];
        ...
        _textField.backgroundColor = [UIColor colorWithRed:224.0/255.0 green:224.0/255.0 blue:224.0/255.0 alpha:1.0];
    }
    return _textField;
}
...
- (UIButton *)iconImgButton
{
    if (!_iconImgButton)
    {
        ...
        _iconImgButton.imageEdgeInsets = UIEdgeInsetsMake(0, 2, 0, 2); // 参数分别是top, left, bottom, right
...
//        _iconCenterImgButton.imageView.contentMode = UIViewContentModeScaleAspectFill;
...
//        NSLog(@"----%f", _textField.frame.size.width);
        ...
        _iconCenterImgButton.frame = CGRectMake(x > 0 ?x:0, 0, self.iconImgButton.frame.size.width, _textField.frame.size.height);
