...
//            }
//        });
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad && preferredStyle == UIAlertControllerStyleActionSheet) {
            alertMaker.popoverPresentationController.sourceView = self.view;
            alertMaker.popoverPresentationController.sourceRect = CGRectMake(0, 0, 1, 1);
        }
        ...
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                [self presentViewController:alertMaker animated:!(alertMaker.jxt_setAlertAnimated) completion:nil];
            });
        }
    }
}
